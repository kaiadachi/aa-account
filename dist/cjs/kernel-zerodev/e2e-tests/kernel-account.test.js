...
describe("Kernel Account Tests", () => {
    ...
    const ownerAccount = (0, accounts_1.mnemonicToAccount)(constants_js_1.OWNER_MNEMONIC);
    const owner = {
        inner: ownerAccount,
        signerType: "aa-sdk-tests",
        signMessage: async (msg) => ownerAccount.signMessage({
            message: { raw: (0, viem_1.toHex)(msg) },
        }),
    ...
    const mockOwner = new mock_signer_js_1.MockSigner();
    const validator = new base_js_1.KernelBaseValidator({
        validatorAddress: config.validatorAddress,
        mode: base_js_1.ValidatorMode.sudo,
        owner,
    });
    const mockValidator = new base_js_1.KernelBaseValidator({
        validatorAddress: config.validatorAddress,
        mode: base_js_1.ValidatorMode.sudo,
        owner: mockOwner,
    });
    ...
    function account(index, owner = mockOwner) {
        ...
        return new account_js_1.KernelSmartContractAccount(accountParams);
    }
    ...
    it("signWithEip6492 should correctly sign the message", async () => {
        ...
        const signature = (0, viem_1.encodeAbiParameters)((0, viem_1.parseAbiParameters)("address, bytes, bytes"), [
